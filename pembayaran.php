<?php 
  ...
  $sql_service = "SELECT *, (SELECT COUNT(nama_service) FROM tb_service) as jumlah_jenis_service FROM tb_detail_transaksi
  ...
  $sql_total_harga = mysqli_query($koneksi, "SELECT SUM(harga*qty) as jml_harga FROM tb_service
  INNER JOIN tb_detail_transaksi ON tb_detail_transaksi.id_service = tb_service.id_service
  ...
  $rows   = [];
  while ($row = mysqli_fetch_array($eksekusi_service)){
        ...
        <div class="row mb-3">
          <div class="col-md-4 mx-2 p-4 rounded text-white" style="background-color: #005082;">
            ...
            <form method="post">
              ...
              <?php if (isset($fetch_transaksi['nama_member'])): ?>
          ...
          <div class="col-md-7 mx-2 p-4 rounded text-white" style="background-color: #005082;">
            <div class="table-responsive">
              ...
              <table class="table text-white">
                ...
                <?php foreach ($rows as $data_service): ?>
                  <tr>
                    <td><?= $i++; ?></td>
                    <td><?= $data_service['nama_service']; ?></td>
                    <td><textarea disabled class="form-control"><?= $data_service['keterangan']; ?></textarea></td>
                    <td class="text-center"><?= $data_service['qty']; ?></td>
                    <td>Rp <?= str_replace(",", ".", number_format($data_service['harga'])); ?></td>
                    <td>Rp <?= str_replace(",", ".", number_format($data_service['subtotal'])); ?></td>
                  </tr>
                <?php endforeach ?>
              </table>
              <hr style="border: 1px solid">
